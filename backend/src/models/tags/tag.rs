...
impl TradeTag {
    ...
    pub async fn create(conn: &Connection, req: CreateTagRequest) -> Result<Self> {
        info!("[TradeTag::create] Starting tag creation with request: category={}, name={}, color={:?}, description={:?}",
        ...
        info!("[TradeTag::create] Preparing INSERT with values: id={}, category={}, name={}, color={:?}, description={:?}, created_at={}, updated_at={}",
        ...
        debug!("  category: {} (type: String, len: {})", category_val, category_val.len());
        debug!("  name: {} (type: String, len: {})", name_val, name_val.len());
        debug!("  color: {:?} (type: Option<&str>)", color_val);
        debug!("  description: {:?} (type: Option<&str>)", desc_val);
        debug!("  created_at: {} (type: String, len: {})", now_val, now_val.len());
        debug!("  updated_at: {} (type: String, len: {})", now_val, now_val.len());
        ...
        match &insert_result {
            Ok(_) => {
                info!("[TradeTag::create] ✓ INSERT executed successfully for tag ID: {}", id);
            },
            Err(e) => {
                error!("[TradeTag::create] ✗ INSERT failed for tag ID {}: {}", id, e);
                error!("[TradeTag::create] INSERT parameters were: id={}, category={}, name={}, color={:?}, description={:?}, created_at={}, updated_at={}",
                    id, req.category, req.name, req.color, req.description, now, now);
        ...
        match Self::find_by_id(conn, &id).await {
            Ok(tag) => {
                info!("[TradeTag::create] ✓ Successfully created and retrieved tag: id={}, category={}, name={}",
                    tag.id, tag.category, tag.name);
                Ok(tag)
            },
            Err(e) => {
                error!("[TradeTag::create] ✗ Failed to retrieve created tag with ID {}: {}", id, e);
                Err(anyhow::anyhow!("Tag was created but failed to retrieve: {}", e))
            }
        }
    }
