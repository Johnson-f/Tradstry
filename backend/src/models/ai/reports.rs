...
pub struct TradingReport {
    ...
    pub behavioral_insights: Vec<BehavioralInsight>,
...
impl TradingReport {
    ...
    pub fn get_total_recommendations(&self) -> usize {
        ...
        self.insights.iter().map(|i| i.recommendations.len()).sum::<usize>() +
        self.behavioral_insights.iter().map(|b| b.recommendations.len()).sum::<usize>()
    }
}
