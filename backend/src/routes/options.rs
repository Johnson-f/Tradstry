...
pub fn configure_options_routes(cfg: &mut web::ServiceConfig) {
    ...
    cfg.service(
        web::scope("/api/options")
            ...
            .route("/analytics/biggest-winner", web::get().to(get_biggest_winner)) // GET /api/options/analytics/biggest-winner?time_range=
            .route("/analytics/biggest-loser", web::get().to(get_biggest_loser)) // GET /api/options/analytics/biggest-loser?time_range=
            .route("/analytics/avg-hold-time-winners", web::get().to(get_avg_hold_time_winners)) // GET /api/options/analytics/avg-hold-time-winners?time_range=
            .route("/analytics/avg-hold-time-losers", web::get().to(get_avg_hold_time_losers)) // GET /api/options/analytics/avg-hold-time-losers?time_range=
            .route("/analytics/risk-reward-ratio", web::get().to(get_risk_reward_ratio)) // GET /api/options/analytics/risk-reward-ratio?time_range=
