...
fn parse_jwt_claims(token: &str) -> Result<SupabaseClaims, AuthError> {
    ...
    if parts.len() != 3 {
        error!("[TradeTags] Invalid JWT format: expected 3 parts, got {}", parts.len());
    ...
    info!("[TradeTags] Successfully parsed claims for user: {}", claims.sub);
    Ok(claims)
}
...
fn extract_token_from_request(req: &HttpRequest) -> Option<String> {
    ...
    info!("[TradeTags] Authorization header present: {}", auth_header.is_some());
    
    if let Some(header_value) = auth_header {
        let header_str = header_value.to_str().ok()?;
        debug!("[TradeTags] Authorization header value (first 30 chars): {}", 
               if header_str.len() > 30 { &header_str[..30] } else { header_str });
...
async fn get_authenticated_user(
    ...
    info!("[TradeTags] ✓ User authenticated successfully: {}", claims.sub);
    Ok(claims)
}
...
async fn get_user_id_from_request(
    ...
    info!("[TradeTags] Extracting user ID from request - path: {}", req.path());
    
    // First, try to get from extensions (if middleware already ran)
    if let Some(claims) = req.extensions().get::<SupabaseClaims>() {
        info!("[TradeTags] ✓ Found SupabaseClaims in extensions - user: {}", claims.sub);
...
pub async fn get_categories(
    ...
    let user_id = match get_user_id_from_request(&_req, &supabase_config).await {
        Ok(id) => {
            info!("[TradeTags] GET /api/trade-tags/categories - User authenticated: {}", id);
            id
        }
        Err(e) => {
            error!("[TradeTags] GET /api/trade-tags/categories - Authentication failed: {}", e);
            return Err(e);
        }
    };
    
    info!("[TradeTags] GET /api/trade-tags/categories - Getting database connection for user: {}", user_id);
    let conn = app_state
    ...
    match TradeTag::get_categories(&conn).await {
        ...
        Err(e) => {
            error!("[TradeTags] GET /api/trade-tags/categories - Failed to get categories: {}", e);
            Ok(HttpResponse::InternalServerError().json(CategoryListResponse {
                success: false,
...
pub async fn get_tags(
    ...
    info!("[TradeTags] GET /api/trade-tags - Starting request, query: {:?}", query);
    let user_id = match get_user_id_from_request(&_req, &supabase_config).await {
        Ok(id) => {
            info!("[TradeTags] GET /api/trade-tags - User authenticated: {}", id);
            id
        }
        Err(e) => {
            error!("[TradeTags] GET /api/trade-tags - Authentication failed: {}", e);
            return Err(e);
        }
    };
    
    info!("[TradeTags] GET /api/trade-tags - Getting database connection for user: {}", user_id);
    let conn = app_state
        .get_user_db_connection(&user_id)
        .await
        .map_err(|e| {
            error!("[TradeTags] GET /api/trade-tags - Failed to get user database connection: {}", e);
            actix_web::error::ErrorInternalServerError("Database connection failed")
        })?
        .ok_or_else(|| {
            error!("[TradeTags] GET /api/trade-tags - User database not found for user: {}", user_id);
            actix_web::error::ErrorNotFound("User database not found")
        })?;
    ...
    match TradeTag::find_all(&conn, Some(query.into_inner())).await {
        ...
        Err(e) => {
            error!("[TradeTags] GET /api/trade-tags - Failed to get tags: {}", e);
...
pub async fn update_tag(
    ...
    let user_id = get_user_id_from_request(&_req, &supabase_config).await?;
    info!("[TradeTags] PUT /api/trade-tags/{{id}} - User authenticated: {}", user_id);
    let tag_id = path.into_inner();
...
pub async fn delete_tag(
    ...
    let user_id = get_user_id_from_request(&_req, &supabase_config).await?;
    info!("[TradeTags] DELETE /api/trade-tags/{{id}} - User authenticated: {}", user_id);
    let tag_id = path.into_inner();
...
pub async fn add_tags_to_stock_trade(
    ...
    let user_id = get_user_id_from_request(&_req, &supabase_config).await?;
    info!("[TradeTags] POST /api/trades/stock/{{id}}/tags - User authenticated: {}", user_id);
    let stock_trade_id = path.into_inner();
...
pub async fn add_tags_to_option_trade(
    ...
    let user_id = get_user_id_from_request(&_req, &supabase_config).await?;
    info!("[TradeTags] POST /api/trades/option/{{id}}/tags - User authenticated: {}", user_id);
    let option_trade_id = path.into_inner();
...
pub async fn remove_tag_from_stock_trade(
    ...
    let user_id = get_user_id_from_request(&_req, &supabase_config).await?;
    info!("[TradeTags] DELETE /api/trades/stock/{{id}}/tags/{{tag_id}} - User authenticated: {}", user_id);
...
pub async fn remove_tag_from_option_trade(
    ...
    let user_id = get_user_id_from_request(&_req, &supabase_config).await?;
    info!("[TradeTags] DELETE /api/trades/option/{{id}}/tags/{{tag_id}} - User authenticated: {}", user_id);
