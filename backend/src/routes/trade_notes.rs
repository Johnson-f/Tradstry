...
pub async fn upsert_trade_note_for_trade(
    ...
    info!("Trade type: {}, Trade ID: {}", path.trade_type, path.trade_id);
    ...
    let trade_exists = match path.trade_type.as_str() {
        "stock" => {
            Stock::find_by_id(&conn, path.trade_id).await
                .map_err(|e| actix_web::error::ErrorInternalServerError(format!("Database error: {}", e)))?
                .map(|s| format!("Stock: {} {}", s.symbol, s.entry_date.to_rfc3339()))
        }
        "option" => {
            OptionTrade::find_by_id(&conn, path.trade_id).await
                .map_err(|e| actix_web::error::ErrorInternalServerError(format!("Database error: {}", e)))?
    ...
    if trade_exists.is_none() {
        return Ok(HttpResponse::NotFound().json(TradeNoteResponse {
            success: false,
            message: format!("Trade not found: {} with ID {}", path.trade_type, path.trade_id),
            data: None,
        }));
    }
    ...
    let note_existed = match path.trade_type.as_str() {
        "stock" => {
            TradeNote::find_by_stock_trade_id(&conn, path.trade_id).await
                .map(|n| n.is_some())
                .unwrap_or(false)
        }
        "option" => {
            TradeNote::find_by_option_trade_id(&conn, path.trade_id).await
                .map(|n| n.is_some())
                .unwrap_or(false)
        }
        _ => false,
...
pub async fn get_trade_note_for_trade(
        ...
        .await
    {
        ...
        Ok(None) => {
            info!("Trade note not found for trade: {} {}", path.trade_type, path.trade_id);
            Ok(HttpResponse::NotFound().json(TradeNoteResponse {
                success: false,
                message: "Trade note not found".to_string(),
                data: None,
...
pub fn configure_trade_notes_routes(cfg: &mut web::ServiceConfig) {
    ...
    cfg.service(
        web::scope("/api/trades")
            .route("/{trade_type}/{trade_id}/notes", web::post().to(upsert_trade_note_for_trade))
            .route("/{trade_type}/{trade_id}/notes", web::get().to(get_trade_note_for_trade))
            .route("/{trade_type}/{trade_id}/notes", web::delete().to(delete_trade_note_for_trade))
    );
}
