...
pub async fn get_stock_by_id(
    ...
    match Stock::find_by_id(&conn, id).await {
        Ok(Some(stock)) => {
            info!("Found stock with ID: {}", id);
...
pub async fn get_all_stocks(
    ...
    match Stock::find_all(&conn, query.into_inner()).await {
        Ok(stocks) => {
            info!("Found {} stocks", stocks.len());
...
pub async fn get_stocks_count(
    ...
    match Stock::count(&conn, &query.into_inner()).await {
        Ok(count) => {
            info!("Total stocks count: {}", count);
            Ok(HttpResponse::Ok().json(ApiResponse::success(serde_json::json!({
                "count": count
            }))))
        }
...
/// Get comprehensive stocks analytics
pub async fn get_stocks_analytics(
    ...
    let total_pnl = Stock::calculate_total_pnl(&conn).await.unwrap_or_default();
    let profit_factor = Stock::calculate_profit_factor(&conn, time_range.clone()).await.unwrap_or_default();
    let win_rate = Stock::calculate_win_rate(&conn, time_range.clone()).await.unwrap_or_default();
    let loss_rate = Stock::calculate_loss_rate(&conn, time_range.clone()).await.unwrap_or_default();
    let avg_gain = Stock::calculate_avg_gain(&conn, time_range.clone()).await.unwrap_or_default();
    let avg_loss = Stock::calculate_avg_loss(&conn, time_range.clone()).await.unwrap_or_default();
    let biggest_winner = Stock::calculate_biggest_winner(&conn, time_range.clone()).await.unwrap_or_default();
    let biggest_loser = Stock::calculate_biggest_loser(&conn, time_range.clone()).await.unwrap_or_default();
    let avg_hold_time_winners = Stock::calculate_avg_hold_time_winners(&conn, time_range.clone()).await.unwrap_or_default();
    let avg_hold_time_losers = Stock::calculate_avg_hold_time_losers(&conn, time_range.clone()).await.unwrap_or_default();
    let risk_reward_ratio = Stock::calculate_risk_reward_ratio(&conn, time_range.clone()).await.unwrap_or_default();
    let trade_expectancy = Stock::calculate_trade_expectancy(&conn, time_range.clone()).await.unwrap_or_default();
    let avg_position_size = Stock::calculate_avg_position_size(&conn, time_range.clone()).await.unwrap_or_default();
    let net_pnl = Stock::calculate_net_pnl(&conn, time_range).await.unwrap_or_default();
...
pub async fn get_total_pnl(
    ...
    match Stock::calculate_total_pnl(&conn).await {
        Ok(pnl) => {
            info!("Total P&L: {}", pnl);
            Ok(HttpResponse::Ok().json(ApiResponse::success(serde_json::json!({
...
pub async fn get_profit_factor(
    ...
    match Stock::calculate_profit_factor(&conn, time_range).await {
        Ok(factor) => {
