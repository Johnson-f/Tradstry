...
use crate::service::brokerage::helpers::{get_existing_account_id, get_existing_holding_id, transaction_exists, get_or_create_snaptrade_user};
...
pub async fn initiate_connection(
    ...
    let data: ConnectBrokerageResponse = serde_json::from_str(&response_text)
        .context(format!("Failed to parse response JSON. Response was: {}", response_text))?;
...
pub async fn get_connection_status(
    ...
    let mut result = stmt.query(libsql::params![connection_id, user_id]).await
    ...
    let response = snaptrade_client
        .call_go_service("GET", &format!("/api/v1/connections/{}/status", snaptrade_connection_id), None::<&Value>, user_id, Some(&user_secret))
        .await
        .context("Failed to call SnapTrade service")?;
...
pub async fn delete_connection(
    ...
    if let Some(snaptrade_conn_id) = snaptrade_connection_id {
        ...
        let _response = snaptrade_client
            .call_go_service("DELETE", &format!("/api/v1/connections/{}", snaptrade_conn_id), None::<&Value>, user_id, Some(&user_secret))
            .await
            .map_err(|e| {
    ...
    ).await.context("Failed to delete connection")?;
...
pub async fn complete_connection_sync(
    ...
    info!("Completing connection sync for user: {}, connection: {}", user_id, connection_id);
        ...
        if status_response.status().is_success() {
            ...
            if let Some(status_str) = status_data.get("status").and_then(|s| s.as_str()) {
                if status_str == "connected" {
                    ...
                    return Err(anyhow::anyhow!("Connection is not ready. Current status: {}", status_str));
                }
            }
        }
    } else if status != "connected" {
        return Err(anyhow::anyhow!("Connection is not connected. Current status: {}", status));
    }
    ...
    for account in sync_data.accounts {
        if let Some(account_obj) = account.as_object()
            && let Some(snaptrade_account_id) = account_obj.get("id").and_then(|v| v.as_str()) {
                ...
                let account_uuid = match get_existing_account_id(conn, connection_id, snaptrade_account_id).await {
                    Some(existing_id) => {
                ...
                let equity_response = snaptrade_client
                    .call_go_service("GET", &format!("/api/v1/accounts/{}/holdings", snaptrade_account_id), None::<&Value>, user_id, Some(&user_secret))
                    .await
                    .ok();
                ...
                let options_response = snaptrade_client
                    .call_go_service("GET", &format!("/api/v1/accounts/{}/holdings/options", snaptrade_account_id), None::<&Value>, user_id, Some(&user_secret))
                    .await
                    .ok();
                ...
                let transactions_response = snaptrade_client
                    .call_go_service("GET", &format!("/api/v1/accounts/{}/transactions?limit=1000&offset=0", snaptrade_account_id), None::<&Value>, user_id, Some(&user_secret))
                    .await
                    .ok();
