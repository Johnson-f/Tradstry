...
use crate::service::brokerage::helpers::{get_existing_account_id, get_existing_holding_id, transaction_exists};
...
pub async fn get_account_detail(
    ...
    let response = snaptrade_client
        .call_go_service("GET", &format!("/api/v1/accounts/{}", snaptrade_account_id), None::<&Value>, user_id, Some(&user_secret))
        .await
    ...
    let account_detail: Value = response.json().await
...
pub async fn sync_accounts(
    ...
    while let Some(row) = result.next().await? {
        ...
        let response = match snaptrade_client
            .call_go_service("POST", "/api/v1/accounts/sync", Some(&sync_req), user_id, None)
            .await
        {
            Ok(resp) => resp,
            Err(e) => {
...
pub async fn get_account_positions(
    ...
    let positions_data: Value = response.json().await
        .context("Failed to parse response")?;
