    ...
    if !response.status().is_success() {
        ...
        if status.as_u16() == 400 {
            ...
            if retry_response.status().is_success() {
                ...
                let retry_error_text = retry_response.text().await.unwrap_or_else(|_| "Unknown error".to_string());
                
                // If user still exists, deletion didn't work
                if retry_error_text.contains("already exist") || retry_status_code == 400 {
                    return Err(anyhow::anyhow!("Cannot delete existing SnapTrade user without credentials. Please contact support to reset your SnapTrade account, or wait a few minutes and try again. Error: {}", retry_error_text));
                }
                
                return Err(anyhow::anyhow!("Failed to recreate SnapTrade user after deletion. Please try again in a few moments. Error: {}", retry_error_text));
            }
        }
        
        return Err(anyhow::anyhow!("Failed to create SnapTrade user: {}", error_text));
    }
