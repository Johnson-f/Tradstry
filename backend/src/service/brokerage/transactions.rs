...
pub async fn get_account_transactions(
    ...
    let mut result = stmt.query(libsql::params![account_id, user_id]).await
        .context("Database query error")?;
...
pub async fn resolve_unmatched_transaction(
    ...
    let mut rows = stmt.query(libsql::params![unmatched_id, user_id]).await
    ...
    let _trade_date: String = row.get(6)
    ...
    let _raw_data: String = row.get(8)
        .context("Failed to get raw_data from unmatched transaction")?;
    let is_option: bool = row.get::<Option<i64>>(9)
    ...
    let trade_id = match action {
        "merge" => {
            ...
            if matched_transaction_id.is_none() {
                return Err(anyhow::anyhow!("matched_transaction_id required for merge action"));
            }
            ...
            return Err(anyhow::anyhow!("Please use the merge_transactions endpoint for merging trades"));
        }
        "create_open" => {
            ...
            // Validate trade type
            if trade_type != "BUY" {
                return Err(anyhow::anyhow!("Only BUY transactions can be created as open positions"));
            }
            ...
            info!("Creating open stock position for unmatched transaction {}: {} shares of {} at ${}", 
            ...
            info!("Successfully resolved unmatched transaction {} to trade ID {}", unmatched_id, trade_id);
            trade_id
        }
        _ => {
            return Err(anyhow::anyhow!("Invalid action. Must be 'merge' or 'create_open'"));
        }
    };
...
pub async fn get_unmatched_suggestions(
    ...
    let mut match_rows = match_stmt.query(libsql::params![user_id, symbol, opposite_type, unmatched_id]).await
