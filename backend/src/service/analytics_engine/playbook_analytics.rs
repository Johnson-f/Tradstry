...
pub async fn calculate_playbook_analytics(
    ...
    let core_metrics = calculate_playbook_core_metrics(
        conn, playbook_id, &time_condition, &time_params
    ).await?;
    
    // Calculate missed trades count
    let missed_trades = calculate_missed_trades_count(
        conn, playbook_id, &time_condition, &time_params
    ).await?;
    
    // Calculate compliance stats
    let compliance_stats = calculate_compliance_stats(
        conn, playbook_id, &time_condition, &time_params
    ).await?;
    ...
    let expectancy = if core_metrics.total_trades > 0 {
        (core_metrics.average_win * (core_metrics.winning_trades as f64 / core_metrics.total_trades as f64)) +
        (core_metrics.average_loss * (core_metrics.losing_trades as f64 / core_metrics.total_trades as f64))
    } else {
        0.0
    };
    
    let fully_compliant_win_rate = if compliance_stats.fully_compliant_trades > 0 {
        let fully_compliant_wins = calculate_fully_compliant_wins(
            conn, playbook_id, &time_condition, &time_params
        ).await?;
...
async fn calculate_compliance_stats(
    ...
    while let Some(row) = rows.next().await? {
        trades_with_compliance.push((
            get_i64_value(&row, 0),
            row.get::<String>(1).unwrap_or_else(|_| "unknown".to_string()),
        ));
    }
    ...
    for (trade_id, trade_type) in trades_with_compliance {
        let compliance_rate = get_trade_compliance_rate(
            conn, trade_id, &trade_type, playbook_id, total_rules
        ).await?;
...
async fn calculate_fully_compliant_wins(
    ...
    let winning_count = if !fully_compliant_stock_trades.is_empty() {
        let placeholders: Vec<String> = fully_compliant_stock_trades.iter()
            .map(|_| "?".to_string())
            .collect();
    ...
    let winning_options_count = if !fully_compliant_option_trades.is_empty() {
        let placeholders: Vec<String> = fully_compliant_option_trades.iter()
            .map(|_| "?".to_string())
            .collect();
