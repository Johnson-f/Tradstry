...
impl TradeVectorization {
    ...
    pub async fn vectorize_trade(
        ...
        let (symbol, analytics) = match trade_type {
            "stock" => {
        ...
        // Step 3: Fetch mistakes from the trade itself
        let mistakes = match trade_type {
            "stock" => {
                Stock::find_by_id(conn, trade_id)
                    .await
                    .map_err(|e| anyhow::anyhow!("Failed to fetch stock for mistakes: {}", e))?
                    .and_then(|s| s.mistakes)
            }
            "option" => {
                OptionTrade::find_by_id(conn, trade_id)
                    .await
                    .map_err(|e| anyhow::anyhow!("Failed to fetch option for mistakes: {}", e))?
                    .and_then(|o| o.mistakes)
            }
            _ => None,
        };
    ...
    fn calculate_trade_analytics_from_stock(stock: &Stock) -> Result<TradeAnalytics> {
        ...
        let net_pnl = match stock.trade_type.to_string().as_str() {
            "BUY" => (stock.exit_price - stock.entry_price) * stock.number_shares - stock.commissions,
            "SELL" => (stock.entry_price - stock.exit_price) * stock.number_shares - stock.commissions,
            _ => 0.0,
        };
        ...
        // Hold time in days
        let hold_time_days = (stock.exit_date.timestamp() - stock.entry_date.timestamp()) as f64
    ...
    fn calculate_trade_analytics_from_option(option: &OptionTrade) -> Result<TradeAnalytics> {
        ...
        // Hold time in days
        let hold_time_days = (option.exit_date.timestamp() - option.entry_date.timestamp()) as f64
            / 86400.0; // seconds in a day
    ...
    fn format_content_for_embedding(
        ...
        content.push_str(&format!("- Net P&L: ${:.2} ({:.2}% return)\n", 
            analytics.net_pnl, analytics.net_pnl_percent));
        
        // Format position size differently for stocks vs options
        if trade_type == "stock" {
            content.push_str(&format!("- Position Size: {:.0} shares (${:.2})\n",
                analytics.position_size / analytics.entry_price, analytics.position_size));
        } else {
            ...
            content.push_str(&format!("- Position Size: {:.0} contracts (${:.2} premium)\n",
                contracts, analytics.position_size));
        }
        
        content.push_str(&format!("- Stop Loss Risk: {:.2}%\n", analytics.stop_loss_risk_percent));
        content.push_str(&format!("- Risk/Reward Ratio: {:.1}:1\n", analytics.risk_to_reward));
        content.push_str(&format!("- Entry: ${:.2}, Exit: ${:.2}\n", 
            analytics.entry_price, analytics.exit_price));
        content.push_str(&format!("- Hold Time: {:.1} days\n\n", analytics.hold_time_days));
