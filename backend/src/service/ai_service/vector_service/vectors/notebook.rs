...
impl NotebookVectorization {
    ...
    pub async fn vectorize_notebook(
        &self,
        user_id: &str,
        note: &NotebookNote,
    ) -> Result<()> {
    ...
    pub async fn delete_notebook_vector(
        &self,
        user_id: &str,
        note_id: &str,
    ) -> Result<()> {
            ...
            arr.iter()
                .filter_map(|v| {
                    if let serde_json::Value::Object(obj) = v {
                        if let Some(serde_json::Value::String(text)) = obj.get("content") {
                            return Some(text.clone());
                        }
                        if let Some(serde_json::Value::Array(content_arr)) = obj.get("content") {
                            let extracted: Vec<String> = content_arr
                                .iter()
                                .filter_map(|c| {
                                    if let serde_json::Value::Object(c_obj) = c {
                                        if let Some(serde_json::Value::String(text)) = c_obj.get("text") {
                                            return Some(text.clone());
                                        }
                                    }
                                    None
