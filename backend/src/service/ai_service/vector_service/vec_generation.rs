...
impl TradeVectorService {
    ...
    pub async fn vectorize_trade_mistakes_and_notes(
        ...
        let mistakes = match trade_type {
            ...
            _ => {
                return Err(anyhow::anyhow!("Invalid trade_type: {}. Must be 'stock' or 'option'", trade_type));
            }
        };
        ...
        log::info!(
            ...
            user_id, trade_id, embedding.len()
        );
    ...
    pub async fn delete_trade_vector(
        &self,
        user_id: &str,
        trade_id: i64,
    ) -> Result<()> {
