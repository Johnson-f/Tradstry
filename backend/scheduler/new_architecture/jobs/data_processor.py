...
class DataProcessor:
    ...
    async def process_fundamental_data(self, raw_data: Dict[str, Any]) -> bool:
        """
        Process and store fundamental data with multi-provider aggregation.
        
        Args:
            raw_data: Raw data from cron job containing fundamental data
            
        Returns:
            True if successful, False otherwise
        """
        try:
            # Check if we have aggregation capability and should use it
            if self.aggregator and raw_data.get('enable_aggregation', True):
                symbols = list(raw_data.get('data', {}).keys())
                logger.info(f"Aggregating fundamental data for {len(symbols)} symbols from multiple providers")
                
                # Use aggregator for multi-provider data
                aggregated_result = await self.aggregator.aggregate_fundamentals(symbols)
                
                if not aggregated_result['success']:
                    logger.warning("Fundamental data aggregation failed, falling back to single provider data")
                    fundamental_data = raw_data.get('data', {})
                else:
                    fundamental_data = aggregated_result['data']
                    logger.info(f"Fundamental data aggregation successful - Coverage: {aggregated_result['coverage']:.1f}%")
                    
                    if aggregated_result['missing_fields']:
                        logger.info(f"Aggregation improved coverage: {len(aggregated_result['missing_fields'])} fields supplemented")
            else:
                # Use single provider data
                fundamental_data = raw_data.get('data', {})
                logger.info(f"Processing fundamental data for {len(fundamental_data)} symbols (single provider)")
            
            if not fundamental_data:
                logger.warning("No fundamental data to process")
                return False
            ...
            for symbol, fund_data in fundamental_data.items():
                ...
                if DataValidator.validate_fundamental_data(fund_dict):
                    processed_fundamentals.append(fund_dict)
                else:
                    logger.warning(f"Skipping {symbol} - incomplete fundamental data")
            
            if not processed_fundamentals:
