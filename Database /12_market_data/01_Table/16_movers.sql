...
CREATE TABLE IF NOT EXISTS market_movers (
    ...
    price DECIMAL(15,4),
    change DECIMAL(15,4),
    percent_change DECIMAL(8,4), -- Store as decimal (e.g., 0.69 for 0.69%)
    
    -- Mover type classification
...
CREATE INDEX IF NOT EXISTS idx_market_movers_percent_change ON market_movers(percent_change DESC);
...
-- Get today's top 10 gainers
-- SELECT * FROM market_movers WHERE mover_type = 'gainer' AND data_date = CURRENT_DATE ORDER BY percent_change DESC LIMIT 10;

-- Get today's top 10 losers
-- SELECT * FROM market_movers WHERE mover_type = 'loser' AND data_date = CURRENT_DATE ORDER BY percent_change ASC LIMIT 10;

-- Get today's most active stocks
-- SELECT * FROM market_movers WHERE mover_type = 'active' AND data_date = CURRENT_DATE ORDER BY fetch_timestamp DESC LIMIT 10;

-- Get all movers for a specific symbol
