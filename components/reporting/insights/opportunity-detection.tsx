...
export function OpportunityDetectionCard({ timeRange = '30d', className }: OpportunityDetectionCardProps) {
  const {
    insights,
    currentInsight,
    loading,
  ...
  // Find opportunity detection insight for the specified time range
  const opportunityDetectionInsight = insights.find(
    insight => insight.insight_type === 'opportunity_detection' && insight.time_range === timeRange
  );

  // Load opportunity detection insight if not already loaded
  useEffect(() => {
    if (opportunityDetectionInsight && !currentInsight) {
      getInsight(opportunityDetectionInsight.id).catch(() => {
        // Error handled by hook
      });
    }
  }, [opportunityDetectionInsight, currentInsight, getInsight]);

  const handleGenerateInsights = async () => {
    try {
      await generateInsights({
        time_range: timeRange,
        insight_type: 'opportunity_detection',
  ...
  const getOpportunityLevelColor = (score: number): string => {
    if (score >= 0.8) return 'text-purple-600 bg-purple-50';
    if (score >= 0.6) return 'text-blue-600 bg-blue-50';
    return 'text-gray-600 bg-gray-50';
  };

  const getOpportunityLevelText = (score: number): string => {
    if (score >= 0.8) return 'High Opportunity';
    if (score >= 0.6) return 'Moderate Opportunity';
    return 'Limited Opportunity';
  };

  if (loading && !opportunityDetectionInsight) {
    return (
      ...
      <Card className={cn("w-full border-red-200", className)}>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <CardTitle className="text-red-700">Opportunity Detection</CardTitle>
            </div>
            <Button 
              variant="outline" 
              size="sm"
        ...
        <CardContent>
          <p className="text-red-600 text-sm">
            Failed to load opportunity detection insight. Please try again.
          </p>
        </CardContent>
      </Card>
    );
  ...
  const insight = currentInsight || opportunityDetectionInsight;
  ...
  return (
    <Card className={cn("w-full", className)}>
      <CardHeader>
        <div className="flex items-start justify-between">
          <div className="space-y-1">
            <CardTitle className="flex items-center gap-2">
              <Search className="h-5 w-5" />
              {insight?.title || 'Opportunity Detection'}
            </CardTitle>
      ...
      <CardContent className="space-y-6">
        ...
        {currentInsight?.confidence_score && (
          <div className="flex items-center justify-between p-4 rounded-lg border">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-medium text-sm">Opportunity Level</p>
                <p className={cn("text-sm font-semibold", getOpportunityLevelColor(currentInsight.confidence_score))}>
                  {getOpportunityLevelText(currentInsight.confidence_score)}
                </p>
              </div>
            </div>
        ...
        {/* Key Opportunity Findings */}
        {currentInsight?.key_findings && currentInsight.key_findings.length > 0 && (
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              <Target className="h-4 w-4 text-purple-600" />
              <h4 className="font-medium text-sm">Opportunity Findings</h4>
            </div>
            <div className="space-y-2">
              {currentInsight.key_findings.map((finding, index) => (
                <div key={index} className="flex items-start gap-2 p-3 bg-purple-50 rounded-lg border border-purple-200">
                  ...
                  <p className="text-sm text-purple-900">{finding}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {currentInsight?.recommendations && currentInsight.recommendations.length > 0 && (
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <h4 className="font-medium text-sm">Opportunity Recommendations</h4>
            </div>
            <div className="space-y-2">
        ...
        {currentInsight?.metadata && (
          <div className="space-y-3 pt-4 border-t">
            <div className="flex items-center gap-2">
              ...
              <h4 className="font-medium text-sm">Opportunity Analysis Details</h4>
            </div>
        ...
        {currentInsight?.confidence_score && currentInsight.confidence_score >= 0.8 && (
          <div className="flex items-center gap-2 p-3 bg-purple-50 border border-purple-200 rounded-lg">
            ...
            <p className="text-sm text-purple-800 font-medium">
              High-value opportunities detected! Consider these recommendations for potential gains.
            </p>
          </div>
        )}
        ...
        {currentInsight?.confidence_score && currentInsight.confidence_score >= 0.6 && currentInsight.confidence_score < 0.8 && (
          <div className="flex items-center gap-2 p-3 bg-blue-50 border border-blue-200 rounded-lg">
            ...
            <p className="text-sm text-blue-800 font-medium">
              Moderate opportunities available. Review findings for potential trading setups.
            </p>
          </div>
        )}
