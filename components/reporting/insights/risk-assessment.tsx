...
export function RiskAssessmentCard({ timeRange = '30d', className }: RiskAssessmentCardProps) {
  const {
    insights,
    currentInsight,
    loading,
  ...
  // Find risk assessment insight for the specified time range
  const riskAssessmentInsight = insights.find(
    insight => insight.insight_type === 'risk_assessment' && insight.time_range === timeRange
  );

  // Load risk assessment insight if not already loaded
  useEffect(() => {
    if (riskAssessmentInsight && !currentInsight) {
      getInsight(riskAssessmentInsight.id).catch(() => {
        // Error handled by hook
      });
    }
  }, [riskAssessmentInsight, currentInsight, getInsight]);
  ...
  if (loading && !riskAssessmentInsight) {
    return (
      ...
      <Card className={cn("w-full border-red-200", className)}>
        ...
        <CardContent>
          <p className="text-red-600 text-sm">
            Failed to load risk assessment insight. Please try again.
          </p>
        </CardContent>
      </Card>
    );
      ...
      <Card className={cn("w-full", className)}>
        ...
        <CardContent>
          <div className="text-center py-8">
            ...
            <h3 className="text-lg font-medium mb-2">No Risk Assessment Found</h3>
  ...
  const insight = currentInsight || riskAssessmentInsight;
  ...
  return (
    <Card className={cn("w-full", className)}>
      ...
      <CardContent className="space-y-6">
          ...
          <div className="flex items-center justify-between p-4 rounded-lg border">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={cn("text-sm font-semibold", getRiskLevelColor(currentInsight.confidence_score))}>
                  {getRiskLevelText(currentInsight.confidence_score)}
                </p>
              </div>
            </div>
        ...
        {currentInsight?.key_findings && currentInsight.key_findings.length > 0 && (
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <h4 className="font-medium text-sm">Risk Findings</h4>
            </div>
            <div className="space-y-2">
              {currentInsight.key_findings.map((finding, index) => (
                <div key={index} className="flex items-start gap-2 p-3 bg-red-50 rounded-lg border border-red-200">
                  ...
                  <p className="text-sm text-red-900">{finding}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {currentInsight?.metadata && (
          <div className="space-y-3 pt-4 border-t">
            <div className="flex items-center gap-2">
              ...
              <h4 className="font-medium text-sm">Risk Analysis Details</h4>
            </div>
        ...
        {/* Risk Warning Banner */}
        {currentInsight?.confidence_score && currentInsight.confidence_score >= 0.7 && (
          <div className="flex items-center gap-2 p-3 bg-red-50 border border-red-200 rounded-lg">
            <TrendingDown className="h-4 w-4 text-red-600" />
            <p className="text-sm text-red-800 font-medium">
              High-risk portfolio detected. Review recommendations immediately.
            </p>
          </div>
        )}
