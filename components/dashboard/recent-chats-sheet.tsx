...
export default function RecentChatsSheet({ children }: RecentChatsSheetProps) {
  ...
  const handleFixMessageCounts = async () => {
    try {
      const response = await fetch('/api/ai/chat/fix-message-counts', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        toast.success(`Message counts fixed for ${result.data.updated_sessions} sessions`);
        refetchSessions(); // Refresh the sessions list
      } else {
        toast.error('Failed to fix message counts');
      }
    } catch (error) {
      toast.error('Failed to fix message counts');
        ...
        <SheetHeader className="pb-4">
          <div className="flex items-center justify-between">
            ...
            <Button 
              variant="outline" 
              size="sm" 
              onClick={handleFixMessageCounts}
              className="text-xs"
            >
              Fix Counts
            </Button>
              ...
              sessions.map((session) => (
                <div key={session.id} className="group p-4 rounded-lg bg-card hover:bg-accent/50 border transition-colors">
                  <div className="flex items-start justify-between mb-2">
                    ...
                    >
                      <h4 className="font-medium text-sm leading-tight truncate cursor-pointer">
