...
import { SetupCreate, SetupCategory, SetupInDB } from '@/lib/types/setups';
import { setupsService } from '@/lib/services/setups-service';
import { toast } from 'sonner';

interface SetupCreateDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSetupCreated: (setup: SetupInDB) => void;
}

export function SetupCreateDialog({ open, onOpenChange, onSetupCreated }: SetupCreateDialogProps) {
  const [loading, setLoading] = useState(false);
  const [formData, setFormData] = useState<Partial<SetupCreate>>({
    name: '',
    description: '',
    category: 'Breakout',
    is_active: true,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const newSetup = await setupsService.createSetup(formData as SetupCreate);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Create New Trading Setup</DialogTitle>
        </DialogHeader>
