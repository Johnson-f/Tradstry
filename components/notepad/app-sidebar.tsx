...
import {
  ...
  useFolders,
  useTemplates,
  useTagsWithCounts as useTags,
  useNotesByTag as useTagNotes,
  useTrash,
...
export function AppSidebar({
  ...
  const currentTagId = activeSlug.startsWith("tag:")
    ? activeSlug.replace("tag:", "")
    : null;
  ...
  const [isTemplatePopoverOpen, setIsTemplatePopoverOpen] =
  ...
  const handleCreateNote = async () => {
    ...
    try {
      const currentFolder = folders?.find((f) => f.slug === activeSlug);
      const folderId =
        currentFolder?.id || folders?.find((f) => f.slug === "home")?.id;
  ...
  const handleDeleteConfirm = async () => {
    ...
    console.log('Deleting item:', itemToDelete); // Debug log

    try {
      switch (itemToDelete.type) {
        case "note":
          // Ensure we pass the correct parameters to match the hook signature
          const result = await deleteNoteMutation.mutateAsync({
            noteId: itemToDelete.id,
            permanent: false // Explicitly set to false for soft delete
          });
      ...
      toast.error(`Failed to delete ${itemToDelete.type}: ${error.message || 'Unknown error'}`);
    }
  };
  ...
  const getActionButton = () => {
    ...
    if (
      ...
      activeSlug === "notes" ||
      (activeFolder &&
        !["favorites", "trash", "tags", "templates"].includes(activeSlug))
    ) {
      return (
  ...
  return (
    <div
      className={`flex h-screen fixed top-0 left-68 z-40 transition-all duration-300 ${
    ...
    >
      ...
      <Sidebar
        collapsible="none"
        className={`flex-1 border-r h-screen overflow-hidden transition-all duration-300 ${
                        ...
                        <div className="flex w-full items-center gap-2">
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              ...
                              <DropdownMenuItem
                                onClick={() =>
                                  openDeleteConfirm("tag", tag.id, tag.name)
                                }
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              <DropdownMenuItem
                                onClick={() =>
                                  startEdit("note", note.id, note.title)
                                }
                              >
                                <Edit className="h-3 w-3 mr-2" />
                                Rename
                              ...
                              <DropdownMenuItem
                                onClick={() =>
                                  openDeleteConfirm("note", note.id, note.title)
                                }
                                className="text-destructive"
                              >
                                <Trash className="h-3 w-3 mr-2" />
