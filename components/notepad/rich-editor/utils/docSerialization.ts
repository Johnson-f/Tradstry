...
export async function docToHash(doc: SerializedDocument): Promise<string> {
  // Check if browser APIs are available (Next.js SSR safe)
  if (typeof window === 'undefined' || 
      typeof CompressionStream === 'undefined' || 
      typeof TextEncoder === 'undefined' || 
      typeof btoa === 'undefined') {
    // Fallback for SSR - return a simple hash
    return `#doc=${Buffer.from(JSON.stringify(doc)).toString('base64')
      .replace(/\//g, '_')
      .replace(/\+/g, '-')
      .replace(/=+$/, '')}`;
  }
...
export async function docFromHash(
  ...
  if (typeof window === 'undefined' || 
    ...
    // Fallback for SSR - simple base64 decode
    try {
      const decoded = Buffer.from(
        m[1].replace(/_/g, '/').replace(/-/g, '+'), 
        'base64'
      ).toString();
