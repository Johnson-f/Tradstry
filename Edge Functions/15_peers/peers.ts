...
// Interface for peer data to be saved
interface PeerData {
  symbol: string;
  name: string;
  price: number;
...
/**
 * Transform API response to peer data
 */
...
/**
 * Validate peer data for basic sanity checks
 */
function validatePeerData(peer: PeerData): boolean {
  ...
  if (peer.price !== undefined && peer.price < 0) return false;
