/**
 * Supabase Edge Function: Sequential Income Statement Data Fetcher
 * 
 * This Edge Function fetches income statement data in a specific sequence:
...
/**
 * Map API breakdown names to database column names for income statements
 */
...
async function fetchIncomeStatementData(
  symbol: string,
  frequency: 'quarterly' | 'annual'
): Promise<IncomeStatementRecord[] | null> {
  try {
    const apiFrequency = frequency === 'quarterly' ? 'quarterly' : 'annual';
    const url = `https://finance-query.onrender.com/v1/financials/${symbol}?statement=income&frequency=${apiFrequency}`;
    
    console.log(`Fetching ${frequency} income statement for ${symbol}...`);
    
    const response = await fetch(url, {
      method: 'GET',
      headers: {
        'Accept': 'application/json',
        'User-Agent': 'Tradistry-SequentialIncomeStatementFetcher/1.0'
      }
    });
    
    if (!response.ok) {
      if (response.status === 404) {
        console.log(`No ${frequency} income statement found for ${symbol}`);
        return null;
      }
    ...
    if (!data || !data.statement || typeof data.statement !== 'object') {
      console.log(`Invalid ${frequency} income statement data for ${symbol}`);
      return null;
    }
    ...
    const records: IncomeStatementRecord[] = [];
    
    for (const fiscalDate of fiscalDates) {
      const record: IncomeStatementRecord = {
    ...
    console.error(`Error fetching ${frequency} income statement for ${symbol}:`, error);
    return null;
  }
}
...
): { recordsToSave: IncomeStatementRecord[]; duplicatesSkipped: number } {
  const recordsToSave: IncomeStatementRecord[] = [];
...
/**
 * Save income statement records to database with duplicate prevention
 */
async function saveIncomeStatementRecords(
  supabase: SupabaseClient,
  records: IncomeStatementRecord[]
  ...
  try {
    ...
    console.error(`Error in saveIncomeStatementRecords:`, error);
    return { success: false, savedCount: 0 };
  }
}

/**
 * Validate income statement records before saving
 */
function validateIncomeStatementRecords(records: IncomeStatementRecord[]): { isValid: boolean; error?: string } {
  ...
  for (const record of records) {
    ...
    const hasFinancialData = Object.keys(record).some(key => 
      ...
      key !== 'fiscal_date' && 
      key !== 'data_provider' && 
      record[key as keyof IncomeStatementRecord] !== null &&
      record[key as keyof IncomeStatementRecord] !== undefined
    );
...
/**
 * Process income statement data for a single symbol and frequency
 */
async function processSymbolIncomeStatement(
  supabase: SupabaseClient,
  symbol: string,
  frequency: 'quarterly' | 'annual',
  startYear: number,
...
📊 Processing ${symbol} (${frequency})...`);
    ...
    const apiRecords = await fetchIncomeStatementData(symbol, frequency);
    if (!apiRecords) {
      ...
      return {
        ...
        frequency,
        status: 'no_data',
        message: `No ${frequency} income statement data available from API`,
        records_saved: 0,
        periods_found: 0,
    ...
    const validation = validateIncomeStatementRecords(recordsToSave);
    ...
    const { success, savedCount } = await saveIncomeStatementRecords(supabase, recordsToSave);
...
🚀 Starting ${frequency.toUpperCase()} PHASE for ${symbolsToProcess.length} symbols`);
  ...
  for (const symbol of symbolsToProcess) {
    let result = await processSymbolIncomeStatement(supabase, symbol, frequency, startYear, endYear);
    ...
    if (result.status === 'error' || result.status === 'no_data') {
        ...
        result = await processSymbolIncomeStatement(supabase, symbol, frequency, startYear, endYear);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`
🚀 STARTING SEQUENTIAL INCOME STATEMENT FETCHER`);
    ...
    // Case 2: User does not provide a list. Automatically determine the next batch.
        ...
        console.log("No specific symbols requested. Automatically determining next batch based on data freshness...");

        const twentyFourHoursAgo = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();
        
        // Fetch recently updated symbols along with their frequency
        ...
        if (fetchError) {
            console.error("Warning: Could not fetch recently updated symbols. Processing first batch as a fallback.", fetchError.message);
        }

        // Track which frequencies are up-to-date for each symbol
        const updatedStatus: { [symbol: string]: { quarterly?: boolean, annual?: boolean } } = {};
        if (recentlyUpdated) {
            for (const r of recentlyUpdated) {
                if (!r.symbol || !r.frequency) continue;
                if (!updatedStatus[r.symbol]) {
                    updatedStatus[r.symbol] = {};
                }
                if (r.frequency === 'quarterly') {
                    updatedStatus[r.symbol].quarterly = true;
                } else if (r.frequency === 'annual') {
                    updatedStatus[r.symbol].annual = true;
                }
            }
        }

        // A symbol is considered "up-to-date" only if all required frequencies have been updated recently.
        // This logic respects the `skipQuarterly` and `skipAnnual` flags.
        const upToDateSymbols = new Set<string>();
        for (const symbol of Object.keys(updatedStatus)) {
            const freqs = updatedStatus[symbol];
            const isQuarterlyDone = skipQuarterly || !!freqs.quarterly;
            const isAnnualDone = skipAnnual || !!freqs.annual;
            if (isQuarterlyDone && isAnnualDone) {
                upToDateSymbols.add(symbol);
            }
        }

        const symbolsNeedingUpdate = allSymbols.filter(s => !upToDateSymbols.has(s));
        
        console.log(`Found ${allSymbols.length} total symbols. ${upToDateSymbols.size} are fully up-to-date. ${symbolsNeedingUpdate.length} symbols require processing.`);
...
${'='.repeat(60)}`);
    ...
    const response = {
      success: true,
      message: 'Sequential income statement data fetch completed for the batch.',
    ...
    return new Response(
      JSON.stringify({
        success: false,
        error: errorMessage,
        message: 'Failed to process sequential income statement data fetch',
        timestamp: getCurrentTimestamp()
      }),
      { 
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
